...
(() => {
  ...
  // ---- pointer-жесty кроп-редактора: панорама картинки, перемещение и
  // ресайз рамки. Один общий pointermove/pointerup на document — жест может
  // увести курсор за пределы маленького холста редактора, а setPointerCapture
  // продолжает присылать события элементу, на котором начался pointerdown,
  // так что слушать можно и там, но document проще для трёх разных источников
  // жеста (canvas/рамка/хэндл) без трёх раздельных наборов capture-логики.
  ...
  // размer (растут вправо/вниз, сужаются к тому же углу — простое, предсказуемое
  // правило вместо «какой угол ресайзили последним»). Описание — в
  // news.cropFrameLabel (title/aria-label на самом элементе, задаётся в HTML
  // и обновляется applyLang()).
