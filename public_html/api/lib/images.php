...
// Потолок стороны картинки новости — производится от свободной ширины,
// которую админ выбрал в редакторе (image_pct, 10..100% ширины карточки), а
// не от фиксированного набора пресетов, как раньше (small/medium/full). С
// этой правки ширина уважается на обеих платформах — телефонный
// @media-оверрайд, форсировавший все размеры на 100%, убран из news.css —
// поэтому нужно взять максимум device-пикселей по каждой платформе и
// зажать его в разумные границы.
//
// Геометрия карточки (public_html/css/news.css) при выбранной ширине pct%:
//   десктоп: .stage 1040, .nw-feed 84.63cqw ≈ 880, минус паддинг .nw-card
//            20px×2 → 840 CSS px содержимого, DPR 2 →
//            840 × pct/100 × 2 = 16.8 × pct device px;
//   телефон: .stage 96vw ≈ 360 CSS, .nw-feed 92cqw ≈ 331 CSS, минус паддинг
//            .nw-card 14px×2 → 303 CSS px содержимого, DPR 3 →
//            303 × pct/100 × 3 = 9.09 × pct device px.
// 16.8 × pct больше 9.09 × pct при любом pct > 0, так что десктопная
// retina-плотность всегда даёт больше device-пикселей, чем телефонная — она
// и определяет потолок.
//
// Округляем вверх (ceil), чтобы не срезать долю честного пикселя, и зажимаем
// в [256, 1280]: 256 — тот же пол, что и у ICON_MAX_SIDE (картинка новости
// не должна декодироваться мельче иконки предмета даже при минимальной
// ширине), 1280 — прежний потолок full-картинки, выше которого ни один
// реальный экран не покажет больше пикселей.
function news_image_cap(int $pct): int {
    return max(256, min(1280, (int)ceil(16.8 * $pct)));
}

// Отображает сырое значение image_pct из тела запроса (int|string|null — оно
// приходит с фронта непровалидированным) на потолок стороны. Нераспознанное
// (не целое число), отсутствующее или вне диапазона [10, 100] значение
// откатывается на news_image_cap(10) — САМЫЙ МАЛЕНЬКИЙ потолок среди
// допустимых значений, а не на самый большой: смысл отката тот же, что и у
// прежнего news_image_max_side() — не разрешить более тяжёлую картинку, чем
// нужно, если фронт прислал не то, чего ждёт сервер.
function news_image_max_side_for_pct($pct): int {
    $n = null;
    if (is_int($pct)) { $n = $pct; }
    elseif (is_string($pct) && ctype_digit($pct)) { $n = (int)$pct; }
    if ($n === null || $n < 10 || $n > 100) { $n = 10; }
    return news_image_cap($n);
}
