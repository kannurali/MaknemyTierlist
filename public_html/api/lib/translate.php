...
// Пустым считается и абзац-отступ из одного неразрывного пробела: редактор
// ставит такой на место лишней пустой строки (NEWSBLOCKS.SPACER), а Google
// отвечает на него пустотой — это считалось бы сбоем, и два отступа подряд
// обрывали перевод всего поста. trim() NBSP не срезает: он работает с байтами.
function tr_spans_empty($spans): bool {
    return preg_match('~^[\s\x{00A0}\x{200B}\x{FEFF}]*$~u', tr_spans_text($spans)) === 1;
}
