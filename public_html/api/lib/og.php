...
// Ревизия ОФОРМЛЕНИЯ картинки: шрифты, марки, фон постера, вёрстка холста.
// Версия в имени файла отвечает только за ДАННЫЕ (rev тирлиста, id+дата
// поста), поэтому смена ассета данные не трогает и старый кэш продолжает
// отдаваться вечно. Так и вышло с marks.png: логотип GLH в нём заменили на
// шута, тирлист подхватил новый (его rev изменился на миграции иконок), а в
// превью новостей ещё месяц висел GLH — посты не редактировались, версия не
// менялась, файл кэша не перестраивался.
//
// Поднимать при ЛЮБОЙ правке того, как картинка выглядит. Старые файлы при
// этом просто перестают адресоваться: деплой ничего не удаляет, вычистить
// images/og/ можно потом руками.
const OG_ASSET_REV = 2;

function og_cache_filename(string $prefix, int $version): string {
    return $prefix . '-r' . OG_ASSET_REV . '-' . $version . '.png';
}
