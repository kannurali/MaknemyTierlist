...
// Через сколько секунд после последнего входа человек перестаёт считаться
// «в сети». Отдельной колонки status нет намеренно: её пришлось бы кому-то
// проставлять и вовремя сбрасывать, а last_login_at уже пишется при каждом
// входе (roblox_touch_user) и врать не может.
 ...
 * адресная форма (@ник). Статус вычисляется из last_login_at, а не хранится.
 */
function chat_user_row(?array $r, int $now): ?array {
    if (!$r) { return null; }
    $seen = (int)($r['last_login_at'] ?? 0);
    return [
...
function chat_users_by_id(PDO $pdo, array $ids, int $now): array {
    ...
    $st = $pdo->prepare(
        "SELECT roblox_id, username, display_name, avatar_url, last_login_at
           FROM users WHERE roblox_id IN ($in)"
    );
    $st->execute($ids);
    $out = [];
