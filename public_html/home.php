...
// Превью главной — статичная карточка assets/og-home.jpg: фирменный первый
// экран (логотип, «Работаем с 2024 года», описание проекта), снятый со
// страницы один раз.
//
// Раньше здесь стояла та же генерируемая картинка, что у /tierlist —
// og_tierlist_image() по строке tierlist. Решение изменено намеренно: корень
// пересылают как ссылку на ПРОЕКТ, и визитка бренда там уместнее витрины
// цен. У /tierlist и /news превью остались живыми (топ-5 предметов с ценами
// и картинка свежего поста), так что расхождение между разделами теперь
// осознанное, а не рассинхрон двух копий одной логики — от чего
// предостерегал комментарий, стоявший здесь раньше.
//
// Побочный эффект, ради которого это стоило сделать так: главная больше не
// ходит в базу вообще. Вместе с запросом ушёл и try/catch вокруг него —
// падать при отрисовке главной стало нечему, а прежняя причина его
// существования (битое превью не должно ронять самую шаримую страницу)
// закрыта тем, что превью теперь просто файл.
//
// Файл обновляется вручную — пересъёмкой первого экрана — и обязан менять
// ?v= при каждой пересъёмке: без нового адреса Telegram и Discord продолжат
// показывать прежнюю картинку из своего кэша, даже если файл на диске уже
// другой.
$ogImage = [
