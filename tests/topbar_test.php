...
// Профиль — пятая копия той же шапки. Именно на ней она однажды и разошлась:
// аватар там был ссылкой, пока остальные четыре несли кнопку, и ни одна
// проверка этого не увидела — profile.php не было ни в одном списке.
$PAGES5 = ['home.php', 'index.php', 'news.php', 'calculator.php', 'profile.php'];
...
test('общие файлы шапки подключены одной версией на всех пяти страницах', function () use ($PUB, $PAGES5) {
...
test('аватар — кнопка меню на всех пяти страницах', function () use ($PUB, $PAGES5) {
    foreach ($PAGES5 as $f) {
        $s = top_header($PUB, $f);
