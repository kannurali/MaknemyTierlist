...
// переписывать проверки, написанные до него; новые смотрят все копии.
//
// Профиль и чат — пятая и шестая копии той же шапки, и разошлась она именно
// на них: на профиле аватар был ссылкой, пока остальные несли кнопку, а чат
// приехал со своей ветки с версиями общих файлов на два шага позади и вовсе
...
// Шапка на всех страницах обязана быть одинаковой — вплоть до номеров версий
// у общих файлов. Разъехавшийся ?v= означает, что часть посетителей осталась
// на старом скрипте: страница отдаётся с no-cache, а js и css — нет.
test('общие файлы шапки подключены одной версией на всех шести страницах', function () use ($PUB, $PAGES6) {
    // Разделитель ~, а не /: в путях к файлам слэши, и со стандартным
    // делителем шаблон разваливался бы на месте первого же из них.
    foreach (['css/base.css', 'css/topbar.css', 'css/design-page.css',
              'js/auth.js', 'js/topbar.js', 'js/i18n.js'] as $asset) {
        $seen = [];
        foreach ($PAGES6 as $f) {
            $page = top_read($PUB . '/' . $f);
            $re = '~' . preg_quote($asset, '~') . '\?v=(\d+)~';
            assert_true((bool)preg_match($re, $page, $m), "$f: подключён $asset с версией");
            if (!$m) { continue; }
            $seen[$m[1]] = true;
        }
        assert_eq(1, count($seen), $asset . ': версия одна на все страницы');
    }
});

// Аватар — кнопка выпадающего меню, а не ссылка: js/topbar.js подменяет его
// то на вход, то на меню пользователя, и href сломал бы обе ветки.
test('аватар — кнопка меню на всех шести страницах', function () use ($PUB, $PAGES6) {
    foreach ($PAGES6 as $f) {
        $s = top_header($PUB, $f);
        assert_true((bool)preg_match(
            '/<button class="mk-avatar" type="button" aria-label="Профиль" data-soon /', $s),
            "$f: аватар должен быть кнопкой");
        assert_eq(0, preg_match('/<a class="mk-avatar"/', $s), "$f: и не ссылкой");
    }
});
...
// «Трейдинг» получил раздел (/trading, см. tests/trading_page_test.php) и
// вышел из разработки так же, как до него «Калькулятор»: пилюля — рабочая
// ссылка той же формы, а не кнопка data-soon.
test('«Трейдинг» в шапке — рабочая ссылка на /trading', function () use ($PUB, $PAGES6) {
    foreach ($PAGES6 as $f) {
        $s = top_header($PUB, $f);
        assert_true((bool)preg_match(
            '/<a class="mk-pill" href="\/trading">\s*<svg viewBox="0 0 18 19"/',
            $s), "$f: «Трейдинг» должен вести на /trading");
...
// Ни «Калькулятора», ни чата, ни «Трейдинга» среди них больше нет — у всех
// есть свои страницы (см. тесты выше). Осталась кнопка профиля, которую
// js/topbar.js по ответу /api/session.php превращает во вход через Roblox
// или в меню пользователя.
test('у каждой кнопки data-soon есть подпись и ключ перевода', function () use ($PUB, $PAGES) {
    ...
    foreach ($PAGES as $f) {
        $s = top_header($PUB, $f);
        assert_eq(1, substr_count($s, 'data-soon'), "$f: только профиль");
        assert_eq(1, substr_count($s, 'data-i18n-title="topbar.soon"'), "$f: ключ на нём");
        assert_eq(1, substr_count($s, 'title="В активной разработке"'), "$f: подпись на нём");
    }
});
...
test('чат — рабочая ссылка на /chat перед профилем', function () use ($PUB, $PAGES6) {
    foreach ($PAGES6 as $f) {
        $s = top_header($PUB, $f);
...
test('js/topbar.js подключён одной версией на всех шести страницах', function () use ($PUB, $PAGES6) {
    $seen = [];
    foreach ($PAGES6 as $f) {
