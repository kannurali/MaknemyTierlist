...
// Эта ветка — не один из вариантов, а тот, что видит БОЙ: profile_trades там
// нет и пока не будет (см. шапку api/profile-stats.php). Значит именно она
// обязана переживать «чужому оборот не отдаётся», а не только ветка с
// настоящей таблицей — та в проде вообще не исполняется.
test('без таблицы чужой профиль тоже не получает оборота', function () {
    $pdo = ps_db();
    ps_user($pdo, PS_OTHER, 'daniktor', 'DANIKTOR');

    [, $peer] = ps_call($pdo, null, null, ['id' => PS_OTHER]);
    assert_eq(false, $peer['available'], 'сделок как будто нет');
    assert_eq(false, array_key_exists('sum', $peer['totals']), 'месячного оборота нет');
    assert_eq(false, array_key_exists('scale', $peer['totals']), 'и шкалы нет');
    foreach ($peer['days'] as $d) {
        assert_eq(false, array_key_exists('sum', $d), 'и в каркасе дней тоже нет');
    }
});
...
// Число сделок работает сигналом доверия, и на чужом профиле оно остаётся.
// Оборот говорит другое — насколько ДОРОГИЕ у человека трейды, — и это ровно
// тот признак, по которому выбирают, кого обманывать.
//
// Проверяем ОТСУТСТВИЕ КЛЮЧА, а не ноль. Ноль — это то же прятание значением,
// от которого избавлена вся остальная страница: он утверждает «оборот есть, и
// он нулевой», выглядит правдой и возвращается одной строкой при следующей
// правке. Тест на `=== 0` остался бы зелёным после такой правки.
test('чужому профилю оборот не отдаётся вовсе', function () {
    $pdo = ps_db();
    ps_user($pdo, PS_OTHER, 'daniktor', 'DANIKTOR');
    ps_make_table($pdo);
    ps_add($pdo, '2026-07-10', 'ok', 500, 7, PS_OTHER);
    ps_add($pdo, '2026-06-10', 'ok', 900, 9, PS_OTHER);

    [, $peer] = ps_call($pdo, '2026-07', null, ['id' => PS_OTHER]);
    assert_eq(7, $peer['totals']['ok'], 'сделки за месяц видны');
    assert_eq(false, array_key_exists('sum', $peer['totals']), 'месячного оборота нет');
    assert_eq(false, array_key_exists('scale', $peer['totals']), 'и шкалы полосы нет');
    assert_eq(16, $peer['lifetime']['ok'], 'счётчик за всё время остаётся');

    // Каркас дня проверять мало: обработчик доливает сумму в день через += уже
    // после каркаса, и срезанный только в одном месте ключ возник бы заново
    // ровно здесь — в дне, где сделки есть.
    assert_eq(7, $peer['days'][9]['ok'], '10-е июля: сделки на месте');
    assert_eq(false, array_key_exists('sum', $peer['days'][9]), 'а подённого оборота нет');
    foreach ($peer['days'] as $d) {
        assert_eq(false, array_key_exists('sum', $d), 'ни в одном дне месяца');
    }
});

// Зеркало предыдущего: правка не должна была отобрать оборот у владельца.
test('свой профиль оборот получает как прежде', function () {
    $pdo = ps_db();
    ps_make_table($pdo);
    ps_add($pdo, '2026-07-10', 'ok', 500, 7);
    ps_add($pdo, '2026-06-10', 'ok', 900, 9);

    [, $mine] = ps_call($pdo, '2026-07');
    assert_eq(3500, $mine['totals']['sum'], 'оборот июля');
    assert_eq(8100, $mine['totals']['scale'], 'шкала — лучший месяц');
    assert_eq(3500, $mine['days'][9]['sum'], 'и подённый оборот');
});

// Главный рабочий случай, который легко сломать «упрощением»: признак
// «чужой» — это сравнение НОМЕРОВ, а не наличие ?id=. Страница печатает
// адресата графика всегда (data-profile на .pf-chart), поэтому свой профиль
// тоже шлёт ?id=. Проверка «параметр задан» отобрала бы оборот у владельца.
test('свой профиль по своему же ?id= оборот не теряет', function () {
    $pdo = ps_db();
    ps_make_table($pdo);
    ps_add($pdo, '2026-07-10', 'ok', 500, 7);

    foreach ([PS_ME, '0' . PS_ME, '00' . PS_ME] as $id) {
        [, $p] = ps_call($pdo, '2026-07', null, ['id' => $id]);
        assert_eq(3500, $p['totals']['sum'], "?id=$id — свой оборот");
        assert_true(array_key_exists('scale', $p['totals']), "?id=$id — и шкала");
    }
});
...
// Защита в глубину, а не только чистый ответ. Мало того что чужие суммы не
// уезжают в JSON — они не должны даже подниматься из базы: SUM(value) в
// основном запросе и целиком запрос «лучший месяц» (для шкалы полосы —
// самое дорогое поле, потолок за всю историю) выполняются только своему.
// Проверка по ЧИСТОМУ ответу этого не поймала бы: обработчик мог бы честно
// стереть sum/scale уже ПОСЛЕ того, как поднял их из базы, и внешне это было
// бы неотличимо от того, что деньги вообще не покидали SQL.
//
// PDO::ATTR_STATEMENT_CLASS подставляет свой класс подготовленных запросов —
// он ничего не меняет в их работе, только записывает текст SQL при каждом
// prepare().
class PsSqlLog extends PDOStatement {
    public static $log = [];
    protected function __construct() { self::$log[] = $this->queryString; }
}

test('запрос лучшего месяца и SUM(value) в базу не уходят на чужом профиле', function () {
    $pdo = ps_db();
    ps_user($pdo, PS_OTHER, 'daniktor', 'DANIKTOR');
    ps_make_table($pdo);
    ps_add($pdo, '2026-07-10', 'ok', 500, 7, PS_OTHER);
    ps_add($pdo, '2026-06-10', 'ok', 900, 9, PS_OTHER);
    ps_add($pdo, '2026-07-10', 'ok', 300, 4);

    $pdo->setAttribute(PDO::ATTR_STATEMENT_CLASS, ['PsSqlLog']);

    PsSqlLog::$log = [];
    ps_call($pdo, '2026-07', null, ['id' => PS_OTHER]);
    foreach (PsSqlLog::$log as $sql) {
        assert_eq(0, substr_count($sql, 'SUM(value)'), "чужой: без SUM(value) — $sql");
    }
    assert_eq(0, substr_count(implode(' ', PsSqlLog::$log), 'GROUP BY substr(day'),
        'чужой: запроса шкалы «лучший месяц» не было вовсе');

    PsSqlLog::$log = [];
    ps_call($pdo, '2026-07');
    $all = implode(' ', PsSqlLog::$log);
    assert_true(substr_count($all, 'SUM(value)') > 0, 'свой: SUM(value) на месте');
    assert_true(substr_count($all, 'GROUP BY substr(day') > 0, 'свой: запрос шкалы выполнен');
});
