...
// Fresh source and target directories for one test, removed afterwards even
// when the test throws halfway, so a red run does not litter the temp dir.
function pub_case(callable $fn): void {
    $src = pub_tmp();
    $dst = pub_tmp();
    try {
        $fn($src, $dst);
...
test('publish copies new files and creates directories', function () {
    pub_case(function (string $src, string $dst) {
...
test('publish reproduces the real public_html byte for byte', function () {
    pub_case(function (string $unused, string $dst) {
