...
// Точек спроса пять: четыре уровня и «оверпрайс». Последняя залита градиентом
// из картинки, поэтому у неё есть файл, класс в CSS и подпись в словаре.
test('точка «оверпрайс» есть в легенде, в модалке и на диске', function () use ($PUB) {
    ...
    assert_eq(['green', 'yellow', 'orange', 'red', 'neon'], $m[1], 'состав и порядок точек спроса');

    $from = strpos($s, '<div class="seg" id="mDemand">');
    $seg  = substr($s, $from, strpos($s, '</div>', $from) - $from);
    preg_match_all('/data-v="([^"]*)"/', $seg, $m);
    assert_eq(['', 'green', 'yellow', 'orange', 'red', 'neon'], $m[1], 'кнопки спроса в модалке');
