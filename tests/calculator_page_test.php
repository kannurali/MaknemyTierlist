...
// калькулятора не должна была случайно расшевелить и её. С появлением
// /trading (tests/trading_page_test.php) пилюля из кнопки data-soon стала
// ссылкой; проверяем форму целиком, а не только адрес.
test('пилюля «Трейдинг» ведёт на /trading', function () use ($PUB) {
    foreach (['home.php', 'index.php', 'news.php', 'calculator.php'] as $f) {
        $s = calc_read($PUB . '/' . $f);
        assert_true((bool)preg_match(
...
// Борта калькулятора увеличены по просьбе владельца. Расти им есть куда
// только вниз: по горизонтали упираются в столбики состояния, и наехать на
// них нельзя — столбики показывают состояние сделки, а не декор.
test('увеличенные борта не наезжают на столбики состояния и на вердикт', function () use ($PUB) {
    $css = calc_read($PUB . '/css/calculator.css');
    ...
    $railW = $num('/\.tc-rail-slot \{[^}]*width: calc\(([\d.]+) \* var\(--u\)\)/s');
    $railH = $num('/\.tc-rail-slot \{[^}]*height: calc\(([\d.]+) \* var\(--u\)\)/s');
    $railT = $num('/\.tc-rail-slot \{[^}]*top: calc\(([\d.]+) \* var\(--u\)\)/s');
    $leftL = $num('/\.tc-rail-slot-l \{ left: calc\(([\d.]+) \* var\(--u\)\)/');
    $leftR = $num('/\.tc-rail-slot-r \{ left: calc\(([\d.]+) \* var\(--u\)\)/');
    $gaugeW = $num('/\.tc-gauge \{[^}]*width: calc\(([\d.]+) \* var\(--u\)\)/s');
    $gaugeL = $num('/\.tc-gauge-l \{ left: calc\(([\d.]+) \* var\(--u\)\)/');
    $gaugeR = $num('/\.tc-gauge-r \{ left: calc\(([\d.]+) \* var\(--u\)\)/');
    $frameH = $num('/\.tc-frame \{[^}]*height: calc\(([\d.]+) \* var\(--u\)\)/s');

    // Больше макетных 248x670 — ради этого правку и делали.
    assert_true($railW > 248 && $railH > 670, 'борт должен быть крупнее макетного');

    // Левый борт кончается до левого столбика, правый начинается после правого.
    assert_true($leftL + $railW < $gaugeL, 'левый борт не должен доходить до столбика');
    assert_true($leftR > $gaugeR + $gaugeW, 'правый борт должен начинаться за столбиком');

    // Поля по краям фрейма одинаковые: борта стоят симметрично.
    assert_eq($leftL, 1443 - ($leftR + $railW), 'борта должны стоять симметрично');

    // Вниз борт тоже не вылезает за фрейм — иначе он наехал бы на подвал.
