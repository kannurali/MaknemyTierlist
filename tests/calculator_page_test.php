...
// «Трейдинг» — именно <span>, а не <a href="#">: ссылка-пустышка выглядела
// рабочей кнопкой и ничего не делала. Проверяем форму, а не только атрибут,
// иначе возврат к якорю прошёл бы незамеченным.
test('пилюля «Трейдинг» остаётся отключённой', function () use ($PUB) {
    foreach (['home.php', 'index.php', 'news.php', 'calculator.php'] as $f) {
        $s = calc_read($PUB . '/' . $f);
        assert_true((bool)preg_match(
            '/<span class="mk-pill" aria-disabled="true"[^>]*>\s*<svg viewBox="0 0 18 19"/',
